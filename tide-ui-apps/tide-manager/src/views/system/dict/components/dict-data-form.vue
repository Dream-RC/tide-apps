...
// 打开弹窗 
const openDataForm = async (type: string, id?: string) => {
...
defineExpose({ openDataForm }) // 提供 open 方法，用于打开弹窗
...
const onSubmit = async () => {
    ...
    try {
        ...
        if (formType.value === 'create') {
            ...
            toast.promise(
                await DictDataApi.createDictData(values as DictDataVo),
                {
                    loading: '创建字典数据中...',
                    success: () => '创建 1 条字典数据成功!',
            ...
            toast.promise(
                await DictDataApi.updateDictData(values as DictDataVo),
                {
                    loading: '更新字典数据中...',
                    success: () => '更新 1 条字典数据成功!',
    ...
    <Dialog :open="dialogVisible" @update:open="(val) => dialogVisible = val">
        <DialogContent>
            ...
            <form id="form-vee-demo" @submit.prevent="onSubmit" class="px-12 no-scrollbar overflow-y-auto">
                ...
                <div class="pb-6">
                    <FormField v-slot="{ componentField }" name="dictType">
                        <FormItem class="">
                            <FormLabel>字典类型</FormLabel>
                            <FormControl>
                                <Input disabled :value="props.dictType" placeholder="请选择字典类型" v-bind="componentField" />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                    </FormField>
                ...
                <div class="pb-6">
                    <FormField v-slot="{ componentField }" name="value">
                        <FormItem class="">
                            ...
                            <FormControl>
                                <Input placeholder="字典键值" v-bind="componentField" />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                    </FormField>
